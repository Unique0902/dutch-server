...
export async function amend(req, res) {
  ...
  await promiseRepository.amend(loc, time, id);
  const fpromiseData = await promiseRepository.findById(id);
  const promiseData = fpromiseData.dataValues;
  return res.status(202).json({ message: `promise amend`, promiseData });
}
