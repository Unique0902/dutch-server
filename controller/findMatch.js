...
export async function take(req, res, next) {
  ...
  const matchData = await findMatchRepository.findById(id);
  if (!matchData) {
    return res.status(401).json({ message: 'Invalid id' });
  }
...
export async function takeByUid(req, res, next) {
  const uid = req.params.uid;
  const matchData = await findMatchRepository.findByUid(uid);
  if (!matchData) {
    return res.status(401).json({ message: 'Invalid uid' });
  }
  res.status(200).json({ data: matchData.dataValues });
