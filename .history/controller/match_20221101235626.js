...
export async function checkMatchingByUid(req, res, next) {
  const uid = req.params.uid;
  const findmatchData = await findMatchRepository.findByUid(uid);
  if (!findmatchData) {
    return res.status(204).json({ message: 'no matching data' });
  }
  const myMatchData = findmatchData.dataValues;
  if (!myMatchData.isMatching) {
    return res.status(204).json({ message: 'no matching' });
  }
  const match = await matchRepository.findById(myMatchData.matchId);
  if (match.dataValues.matchData1Id != myMatchData.id) {
    const opponentMatchData = await findMatchRepository.findById(
      match.dataValues.matchData1Id
    ).dataValues;
    res.status(200).json({ myMatchData, opponentMatchData });
  } else {
    const opponentMatchData = await findMatchRepository.findById(
      match.dataValues.matchData2Id
    ).dataValues;
    res.status(200).json({ myMatchData, opponentMatchData });
  }
}
