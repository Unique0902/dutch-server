...
export async function updatePraiseData(req, res) {
  ...
  if (!userData.praiseData) {
    ...
    if (fpraiseData.matchData[roomId]) {
      return res.status(403).json({ message: `praiseData exist already` });
    }
...
export async function amendPraiseData(req, res) {
  const { userId, isChecked1, isChecked2, isChecked3, roomId } = req.body;
  const fuser = await userRepository.findByUid(userId);
  if (!fuser) {
    return res.status(404).json({ message: `user not exists` });
  ...
  const userData = fuserData.dataValues;
  if (!userData.praiseData) {
    return res.status(404).json({ message: `praiesData not exists` });
  } else {
    const fpraiseData = JSON.parse(userData.praiseData);
    if (fpraiseData.matchData[roomId]) {
      const {
        roomId: roomId2,
        isChecked1: isChecked12,
        isChecked2: isChecked22,
        isChecked3: isChecked32,
      } = fpraiseData.matchData[roomId];
      console.log(roomId2, isChecked12);
      let praise1 = isChecked12 ? fpraiseData.praise1 - 1 : fpraiseData.praise1;
      let praise2 = isChecked22 ? fpraiseData.praise2 - 1 : fpraiseData.praise2;
      let praise3 = isChecked32 ? fpraiseData.praise3 - 1 : fpraiseData.praise3;
      praise1 = isChecked1 ? praise1 + 1 : praise1;
      praise2 = isChecked2 ? praise2 + 1 : praise2;
      praise3 = isChecked3 ? praise3 + 1 : praise3;
      const praiseData = JSON.stringify({
        praise1,
        praise2,
        praise3,
      ...
      return res
        .status(404)
        .json({ message: `praiesData in roomId not exists` });
    }
  }
  res.status(201).json({ message: 'amend praiseData successfully' });
}
...
export async function updateBlameData(req, res) {
  ...
  if (!userData.blameData) {
    ...
    if (fblameData.matchData[roomId]) {
      return res.status(403).json({ message: `blameData exist already` });
    }
