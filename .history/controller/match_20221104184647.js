...
export async function end(req, res, next) {
  ...
  await chattingRoomRepository.updateChattingRoomById(roomId, beforeMatchId);
  ...
  console.log(roomId);
  getSocketIo().in(roomId.toString()).emit('match-delete', beforeMatchId);
  await ffindMatch1.destroy();
  await ffindMatch2.destroy();
  res.status(200).json({ beforeMatchId });
}
