...
export async function removeByUid(uid) {
  const findMatch = await BeforeMatchData.findOne({ where: { uid } });
...
export async function removeById(id) {
  const findMatch = await BeforeMatchData.findOne({ where: { id } });
  await findMatch.destroy();
  return true;
}

export async function create(findMatch) {
  return BeforeMatchData.create(findMatch).then((data) => data.dataValues.id);
}
